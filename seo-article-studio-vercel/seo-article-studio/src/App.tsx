  ...
  for (let i = 0; i <= delays.length; i++) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          detail || `Status server: ${response.status}`
        );
      }
      ...
      if (i === delays.length) {
        throw new Error(
          error?.message ||
            "Gagal terhubung ke AI. Silakan coba beberapa saat lagi."
        );
      }
      ...
      {blocks.map((block, idx) => {
        ...
        if (trimmed.startsWith('# ')) {
          return (
            <h1
              key={idx}
              className="text-2xl font-bold text-emerald-400 mt-6 mb-2"
            >
              {trimmed.replace('# ', '')}
            </h1>
          );
        ...
        if (trimmed.startsWith('## ')) {
          return (
            <h2
              key={idx}
              className="text-lg font-bold text-emerald-300 mt-5 mb-2 border-b border-slate-800 pb-1"
            >
              {trimmed.replace('## ', '')}
            </h2>
          );
        ...
        if (trimmed.startsWith('### ')) {
          return (
            <h3
              key={idx}
              className="text-base font-semibold text-slate-100 mt-4 mb-1"
            >
              {trimmed.replace('### ', '')}
            </h3>
          );
        ...
        if (trimmed.startsWith('> ')) {
          return (
            <blockquote
              key={idx}
              className="p-3 my-2 border-l-4 border-emerald-500 bg-emerald-950/30 rounded-r-xl text-emerald-200 text-xs italic"
            >
              {trimmed.replace('> ', '')}
            </blockquote>
          );
...
export default function App() {
  ...
  const [isAnalyzingSubtopics, setIsAnalyzingSubtopics] =
    useState(false);

  const [ideaGenerationCount, setIdeaGenerationCount] =
    useState(0);
  ...
  const [refinementInstruction, setRefinementInstruction] =
    useState('');
  ...
  const stepsList = [
    ...
    {
      id: 5,
      title: 'Penulisan Draf',
      icon: Cpu,
      short: 'Draft'
    },
    {
      id: 6,
      title: 'Audit E-E-A-T Google',
      icon: UserCheck,
      short: 'E-E-A-T Google'
    },
    {
      id: 7,
...
- relevan dengan artikel`;
  ...
  const analyzeTopicPotential = async (
    ...
    if (!generalTopic.trim()) {
      setErrorMessage(
        "Silakan masukkan topik besar atau niche terlebih dahulu."
      );
      return;
    }
    ...
    const currentBatch = isRegenerate
    ...
    const accumulatedTitles = isRegenerate
      ? previousSubtopics
      : [];

    const avoidPrompt =
      accumulatedTitles.length > 0
        ? `

PERINGATAN SANGAT PENTING:
JANGAN PERNAH MENGULANG atau menampilkan kembali ide-ide subtopik yang sudah dihasilkan sebelumnya berikut ini:
...
Hasilkan 10 subtopik BARU dan BENAR-BENAR UNIK yang belum ada di daftar di atas.`
    ...
    try {
      const prompt = `Pengguna memberikan topik besar/niche berikut:

"${generalTopic.trim()}"

Tugasmu adalah menganalisis dan menghasilkan 10 pilihan ide pembahasan (subtopik spesifik) yang berbeda dan segar (Variasi Batch Ke-${currentBatch}) yang paling berpotensi banyak dicari di Google, bernilai SEO tinggi, serta ramah monetisasi AdSense.
...

{
  "ideas": [
    {
      ...
      const resultText = await callGeminiApi(
        prompt,
        "Kamu adalah SEO Riset Analyst & Trend Specialist Google Global & Indonesia."
      );
      ...
      if (
        ...
        const newTitles = parsed.ideas.map(
          item => item.title
        );

        setPreviousSubtopics(prev =>
          isRegenerate
            ? [...prev, ...newTitles]
            : newTitles
        );
        ...
        throw new Error(
          "Format analisis topik tidak valid."
        );
      }
    } catch (err) {
      setErrorMessage(
        "Gagal menganalisis potensi topik: " +
          err.message
      );
    } finally {
  ...
  const handleCustomTopicSubmit = () => {
    if (!customSpecificTopic.trim()) {
      setErrorMessage(
        "Silakan isi topik spesifik Anda terlebih dahulu."
      );
      return;
    }
    ...
    setErrorMessage('');
    setSelectedSubtopic(
      customSpecificTopic.trim()
    );
    setPageStage('customize_details');
  };

  const startArticleGeneration = async (
    isAlternative = false
  ) => {
    ...
    try {
      setAutoStatus(
        isEnglish
          ? "Memproses riset keyword SEO & search intent..."
          : "Memproses riset keyword SEO & niat pencarian pembaca..."
      );

      const prepPrompt = `Lakukan riset SEO mendalam dan pilihkan kata kunci terbaik untuk topik spesifik berikut:

Topik Spesifik Terpilih:
"${targetTopic}"

Konteks Topik Induk:
"${generalTopic}"

Bahasa Target Output Artikel:
...

{
  ...
  "internalLinkSuggestions": [
    "ide artikel internal 1",
    "ide artikel internal 2"
  ],
  "externalLinkSuggestions": [
    "rujukan sumber data publik/otoritas 1",
    "rujukan otoritas 2"
  ],
  "outline": "Struktur heading (H1, H2, H3, FAQ) yang rapi"
}`;

      const prepResult = await callGeminiApi(
        prepPrompt,
        `Kamu adalah SEO Data Analyst & Content Strategist Senior. ${langInstruction}`
      );

      const cleanPrep = prepResult
        .replace(/```json/g, '')
        .replace(/```/g, '')
        .trim();
      ...
      try {
        ...
        prepData = {
          ...
          longTailKeywords: isEnglish
            ? `guide to ${targetTopic}`
            : `panduan ${targetTopic}`,

          lsiKeywords: isEnglish
            ? `solutions for ${targetTopic}`
            : `solusi ${targetTopic}`,

          targetAudience: isEnglish
            ? `Readers looking for solutions.`
            : `Pembaca yang mencari solusi.`,

          painPoints: isEnglish
            ? `Difficulty finding clear guidance.`
            : `Kesulitan menemukan penjelasan praktis.`,

          keyQuestions:
            `1. How to start?\n2. What are key tips?`,

          uniqueAngle: isEnglish
            ? `A direct, step-by-step practical approach.`
            : `Pendekatan langsung pada langkah-langkah solutif.`,
      ...
      setAutoStatus(
        isEnglish
          ? `Menulis artikel (~${optionalData.targetWordCount} kata)...`
          : `Menulis artikel & ramah pembaca (~${optionalData.targetWordCount} kata)...`
      );

      const userExpPrompt =
        optionalData.userExperience.trim()
      ...
      const alternativeVariation = isAlternative
        ? `Pastikan sudut pandang, gaya pembuka, dan struktur paragraf berbeda dari artikel standar namun tetap membahas topik yang sama secara mendalam.`
        : '';
...
2. Keyword Utama:
"${prepData.primaryKeyword}"

3. Long-tail Keywords:
"${prepData.longTailKeywords}"

4. Tone:
"${optionalData.toneOfVoice}"

5. Target Kata:
MINIMAL ${optionalData.targetWordCount} KATA.
...
${prepData.outline}`;
      ...
      const articleText = await callGeminiApi(
        writePrompt,
        `Kamu adalah Penulis Blog Manusia Senior. ${langInstruction}`
      );

      const metaPrompt = `Buatkan Meta Title SEO (maks 60 char) dan Meta Description (maks 155 char) dalam ${
        isEnglish
          ? 'English'
          : 'Bahasa Indonesia'
      } untuk:
...
${targetTopic}`;
      ...
      const metaText = await callGeminiApi(
        metaPrompt,
      ...
      let metaDesc =
        `Panduan mendalam mengenai ${targetTopic}.`;
      ...
      setAutoStatus(
        isEnglish
          ? "Menjalankan audit On-Page SEO & E-E-A-T..."
          : "Menjalankan audit On-Page SEO, E-E-A-T, & AdSense..."
      );

      const auditPrompt = `Evaluasi artikel berikut berdasarkan standar Google E-E-A-T, On-Page SEO, dan AdSense.
...

{
  ...
  "adsenseFeedback": [
    "Siap monetisasi.",
    "Bebas low value content."
  ]
}`;

      const auditResult = await callGeminiApi(
        auditPrompt,
        "Kamu adalah Google Search Quality Rater."
      );
      ...
      setResultData({
        ...
        targetAudience:
          prepData.targetAudience || '',

        painPoints:
          prepData.painPoints || '',

        keyQuestions:
          prepData.keyQuestions || '',

        uniqueAngle:
          prepData.uniqueAngle || '',
        ...
        lsiKeywords:
          prepData.lsiKeywords || '',

        searchIntent:
          prepData.searchIntent ||
          'Informational',
        ...
        internalLinkSuggestions:
          prepData.internalLinkSuggestions ||
          [],

        externalLinkSuggestions:
          prepData.externalLinkSuggestions ||
          [],
        ...
        eeatScore:
          auditData.eeatScore || 93,

        eeatFeedback:
          auditData.eeatFeedback ||
          ["Memenuhi standar."],

        qualityScore:
          auditData.qualityScore || 92,

        qualityFeedback:
          auditData.qualityFeedback ||
          ["Berkualitas tinggi."],

        seoScore:
          auditData.seoScore || 94,
        ...
        adsenseScore:
          auditData.adsenseScore || 95,

        adsenseFeedback:
          auditData.adsenseFeedback ||
          ["Siap AdSense."]
      });
...
Perbarui atau modifikasi artikel di atas sesuai instruksi spesifik pengguna tersebut.

Pertahankan:
- kualitas SEO
- kealamian bahasa human-like
- struktur artikel
- format heading markdown yang rapi
- informasi utama artikel

Keluarkan artikel lengkap terbaru hasil revisi secara utuh tanpa teks pembuka/penutup lain.

${langPrompt}`;

    const summaryPrompt = `Berdasarkan instruksi pengguna berikut:

"${refinementInstruction.trim()}"

Buatkan 2-3 poin penjelasan singkat dan jelas dalam bahasa Indonesia mengenai apa saja bagian yang diubah atau ditambahkan pada artikel.`;
    ...
    try {
      const [
        ...
        callGeminiApi(
          prompt,
          "Kamu adalah editor senior dan spesialis konten SEO."
        ),

        callGeminiApi(
          summaryPrompt,
          "Kamu adalah asisten editor."
        )
      ]);
        ...
        setLastRefinementInfo({
          ...
          explanation:
            summaryResult
              ...
              .replace(/\*\*/g, '')
              .replace(/\*/g, '') ||
            "Perubahan telah diterapkan sesuai dengan instruksi Anda."
        });
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 overflow-x-auto shadow-2xl">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-3">
              ...
              <span className="text-xs font-mono text-emerald-400">
                App.tsx (React Single Component)
              </span>
...
// Prompt gambar dibuat menggunakan Gemini Text API.`}
              ...
              <div className="space-y-6 my-2">
                ...
                {isAnalyzingSubtopics && (
                  ...
                  <div className="p-8 bg-slate-800/60 border border-slate-700/80 rounded-3xl text-center space-y-3">
                    ...
                    <p className="text-sm text-slate-200 font-medium">
                      Memindai 10 ide subtopik spesifik yang paling banyak dicari di Google...
                    </p>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-700 pb-3 gap-2">
                        ...
                        <div>
                          ...
                          <h3 className="text-lg font-bold text-emerald-400 flex items-center gap-2">
                            ...
                            Rekomendasi Subtopik Potensial
                          </h3>

                          <p className="text-xs text-slate-400">
                            Klik salah satu pilihan di bawah untuk melanjutkan.
                          </p>
                        ...
                        >
                          ...
                          Cari 10 Ide Lainnya (Tanpa Mengulang)
                        </button>
                ...
                <div className="bg-slate-800/40 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-3">
                  ...
                  <label className="block text-xs font-bold uppercase tracking-wider text-slate-200 flex items-center gap-2">
                    ...
                    Punya Subtopik Spesifik Sendiri?
                  </label>
              ...
              <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur space-y-6 my-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <label className="block text-xs font-semibold text-slate-300 flex items-center gap-1.5">
                      ...
                      Bahasa Target Output
                    </label>

                    <select
                      value={
                        optionalData.targetLanguage
                      }
                      onChange={e =>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-2xl px-4 py-3 text-slate-200 text-xs focus:outline-none focus:border-emerald-500"
                    >
                      <option value="Santai & Akrab (Ramah Pembaca)">
                        Santai & Akrab (Ramah Pembaca)
                      </option>

                      <option value="Profesional & Otoritatif (Pakar)">
                        Profesional & Otoritatif (Pakar)
                      </option>

                      <option value="Informatif & Edukatif (Step-by-Step)">
                        Informatif & Edukatif (Step-by-Step)
                      </option>

                      <option value="Persuasif & Menjual (Copywriting)">
                        Persuasif & Menjual (Copywriting)
                      </option>
                    </select>
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      <option value="800">
                        ~800 Kata (Ringkas & Padat)
                      </option>

                      <option value="1200">
                        ~1200 Kata (Standar SEO Optimal)
                      </option>

                      <option value="1800">
                        ~1800 Kata (Long-form Mendalam)
                      </option>
                    </select>
                  ...
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <label className="block text-xs font-semibold text-slate-300">
                      Catatan Pengalaman / Sudut Pandang Unik (Opsional)
                    </label>
                ...
                <div className="pt-4 border-t border-slate-700/60 flex justify-end">
                  ...
                  >
                    ...
                    Jalankan Generator 9 Langkah Otomatis
                  </button>
              ...
              <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-10 shadow-2xl backdrop-blur text-center space-y-6 my-12">
                ...
                <div className="space-y-2 max-w-md mx-auto">
                  ...
                  <h3 className="text-xl font-bold text-slate-100">
                    Sedang Membuat Artikel SEO Pro
                  </h3>
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <div className="p-4 bg-slate-900 border border-slate-700 rounded-2xl space-y-2">
                      ...
                      <span className="text-xs font-bold text-sky-400 uppercase tracking-wider block">
                        Pertanyaan Paling Sering Dicari (Key Questions)
                      </span>

                      <p className="text-xs sm:text-sm text-slate-200 leading-relaxed whitespace-pre-line">
                        {resultData.keyQuestions}
                      </p>
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                      ...
                      <div className="p-4 bg-slate-900 border border-slate-700 rounded-2xl space-y-2">
                        ...
                        <span className="text-xs font-bold text-teal-400 uppercase tracking-wider block">
                          LSI / Konteks Keywords
                        </span>

                        <p className="text-xs sm:text-sm text-slate-200">
                          {resultData.lsiKeywords}
                        </p>
                ...
                {currentStep === 4 && (
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <div className="p-4 bg-slate-900 border border-slate-700 rounded-2xl space-y-2">
                      ...
                      <pre className="text-xs text-slate-200 font-mono whitespace-pre-line leading-relaxed">
                        {resultData.outline}
                      </pre>
                ...
                {currentStep === 5 && (
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <h3 className="text-lg font-bold text-emerald-400 flex items-center gap-2">
                      ...
                      Penulisan Draf & Human-like Review
                    </h3>

                    <p className="text-xs text-slate-400">
                      Artikel telah ditulis dengan standar human-like dan mengalir alami.
                    </p>
                ...
                {currentStep === 6 && (
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <ul className="space-y-2 pt-2">
                      ...
                      {resultData.eeatFeedback.map(
                        (fb, i) => (

                          <li
                            key={i}
                            className="text-xs sm:text-sm bg-slate-900 border border-slate-700 p-3 rounded-xl text-slate-200 flex items-start gap-2"
                          >
                ...
                {currentStep === 8 && (
                  ...
                  <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                    ...
                    <ul className="space-y-2 pt-2">
                      ...
                      {resultData.adsenseFeedback.map(
                        (fb, i) => (
                  ...
                  <div className="space-y-6">
                        ...
                        <div className="bg-slate-900 border border-emerald-500/40 p-4 rounded-2xl flex items-start gap-3 shadow-md mt-6">
                          ...
                          <div className="space-y-1.5 text-xs">
                            ...
                            <p className="text-slate-300">
                              ...
                              "{lastRefinementInfo.instruction}"
                            ...
                            <div className="text-slate-200 bg-slate-950 border border-slate-800 p-3 rounded-xl mt-2 space-y-1">
                              ...
                              <span className="text-emerald-400 font-semibold block">
                                Detail yang Telah Diubah:
                              </span>

                              <p className="leading-relaxed whitespace-pre-line">
                                {
                                  lastRefinementInfo.explanation
                                }
                              </p>
                    ...
                    <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl">
                      ...
                      <div className="flex items-center space-x-2 border-b border-slate-700 pb-3">
                        ...
                        <h3 className="text-lg font-bold text-slate-100">
                          Ubah & Sesuaikan Bagian Tertentu (AI Editor)
                        </h3>

                      </div>

                      <p className="text-xs text-slate-400">
                        Ingin mengubah nada, memperpanjang paragraf tertentu, menambahkan tips khusus, atau memperbaiki sub-bagian tertentu? Tulis instruksi Anda di bawah.
                      </p>
      ...
      <footer className="mt-auto border-t border-slate-800 bg-slate-950 py-4 px-4 text-center text-xs text-slate-400 space-y-1">
        ...
        <p>
          Hak cipta tools oleh Novarty - Grow from Home. Dilindungi Undang-Undang.
        </p>

        <p>
          Bila ada error, kendala, atau masukan untuk pengembangan tools, email ke novarty@gmail.com.
        </p>
